...
func readLines(path string) ([]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("errors: file %s not found", path)
	}
...
func sourceRows(rows []string, frame Frame, before, after int, colorized bool) []string {
	...
	if len(lines) < frame.Line {
		message := fmt.Sprintf(
			"errors: too few lines, got %d, want %d",
...
func sprint(err error, nums []int, colorized bool) string {
	...
	for _, frame := range frames {
		...
		if strings.Contains(message, "ocean/") || strings.Contains(message, "go/server") {
			if colorized {
