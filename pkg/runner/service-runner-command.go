...
func (serviceRunner *ServiceRunner) execCommand() {
	if serviceRunner.IsRunning {
		serviceRunner.runner.logs.SendLog(aurora.Yellow("Waiting for old process to be killed...").String(), serviceRunner.service.Name)
		time.Sleep(time.Millisecond * 100)
		serviceRunner.execCommand()
		return
	}

	serviceRunner.runner.logs.SendLog(aurora.Yellow("Starting process...").String(), serviceRunner.service.Name)
	...
	if serviceRunner.service.Cwd != "" {
		serviceRunner.cmd.Dir = serviceRunner.basePath + "/" + serviceRunner.service.Cwd
	}
	...
	for {
		...
		message := string(line)
		serviceRunner.runner.logs.SendLog(message, serviceRunner.service.Name)
	}
